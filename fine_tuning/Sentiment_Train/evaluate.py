...
def extract_predictions_and_surfaces(tuner_obj, data_loader, seed=1234):
    ...
    with torch.no_grad():
        for market_iv_surface, market_price_surface, context in data_loader:
            ...
            context              = context.to(tuner_obj.device, dtype=torch.float32)

            scaled_input = tuner_obj.scale_input(market_iv_surface)
            _, predicted_params = tuner_obj.model(scaled_input, context)
...
def _evaluate_one(model_name, date_range, MODEL_REGISTRY, data_cache):
    ...
    if not os.path.isdir(model_dir):
        print(f"  [SKIP] No model directory: {model_dir}")
        return None

    # Find the best-epoch checkpoint (best_model_epoch_*.pth)
    candidates = sorted(glob.glob(os.path.join(model_dir, "best_model_epoch_*.pth")))
    if not candidates:
        # Fall back to plain best_model.pth for backwards compatibility
        fallback = os.path.join(model_dir, "best_model.pth")
        if os.path.exists(fallback):
            candidates = [fallback]
    if not candidates:
        print(f"  [SKIP] No checkpoint found in: {model_dir}")
        return None
    checkpoint_path = candidates[-1]  # highest epoch number
    ...
    context_tensor    = data_cache["context_tensor"]
    ...
    if RUN_ZERO_SHOT_COMPARISON:
        ...
        _, actual_zero, pred_zero = extract_predictions_and_surfaces(
            zero_tuner,
            DataLoader(TensorDataset(real_iv_tensor, real_price_tensor, context_tensor),
                       batch_size=16, shuffle=False),
            seed=1234
        )
...
if __name__ == "__main__":
    ...
    for date_range in CANDIDATE_DATE_RANGES:
        ...
        iv, price, ctx, dates, lm_conv = create_iv_and_price_grids_from_raw(
            spx_files, max_days=None, use_calls_only=False,
            min_points_per_day=6, min_tau_per_day=2, min_lm_per_day=3
        )
        iv, price, ctx, dates = filter_extreme_prices(
            iv, price, ctx, dates,
    ...
    # Evaluate all models x all date ranges
    all_metrics = {}
