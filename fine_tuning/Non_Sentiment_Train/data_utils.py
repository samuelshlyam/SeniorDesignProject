...
def filter_extreme_prices(iv_tensor, price_tensor, dates,
...
def create_iv_and_price_grids_from_raw(
    ...
    return iv_tensor, price_tensor, used_dates, lm_convention
...
def build_realworld_loaders(real_iv_tensor, real_price_tensor, batch_size=16, val_fraction=0.15, seed=42):
    dataset = TensorDataset(real_iv_tensor, real_price_tensor)
    total_size = len(dataset)
...
def build_model_from_config(model_config, device):
    model = VolatilitySurfaceCNN(param_bounds=model_config["param_bounds"]).to(device)
...
def run_zero_shot_evaluation(model_config, real_iv_tensor, real_price_tensor,
                             lm_convention, device, offline_weight=0.70):
