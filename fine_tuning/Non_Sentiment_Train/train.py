...
def _save_history_npz(history_dict, save_path):
    ...
    np.savez(
        save_path,
        train_abs=np.array(history_dict.get("train_abs", []), dtype=np.float64),
        train_rel=np.array(history_dict.get("train_rel", []), dtype=np.float64),
        val_abs=_none_to_nan(history_dict.get("val_abs", [])),
        val_rel=_none_to_nan(history_dict.get("val_rel", [])),
        lr=np.array(history_dict.get("lr", []), dtype=np.float64),
    )
...
def run_training():
    ...
    for DATE_RANGE in CANDIDATE_DATE_RANGES:
        ...
        real_iv_tensor, real_price_tensor, real_dates, lm_convention = \
            create_iv_and_price_grids_from_raw(
                spx_files,
                max_days=None,
        ...
        real_iv_tensor, real_price_tensor, real_dates = filter_extreme_prices(
            real_iv_tensor, real_price_tensor, real_dates,
        ...
        if RUN_ZERO_SHOT_COMPARISON:
            ...
            for model_name in CANDIDATE_MODELS:
                ...
                z = run_zero_shot_evaluation(
                    cfg, real_iv_tensor, real_price_tensor,
        ...
        for selected_model_name in CANDIDATE_MODELS:
            ...
            train_loader, val_loader = build_realworld_loaders(
                real_iv_tensor, real_price_tensor,
            ...
            final_model_path = os.path.join(model_output_dir, "final_model.pth")
            torch.save(model.state_dict(), final_model_path)

            history_npz_path = os.path.join(model_output_dir, "training_history.npz")
            _save_history_npz(history, history_npz_path)
