...
def _to_1d_strike_tensor(K_strikes, device):
    if torch.is_tensor(K_strikes):
        return K_strikes.to(device=device, dtype=torch.float64).reshape(-1)
    return torch.as_tensor(K_strikes, dtype=torch.float64, device=device).reshape(-1)
...
def Heston_MC_Batched(kappa, theta, sigma, v0, rho, T, K_strikes, S, r, q, N_paths=2000, N_steps=100):
    ...
    kappa = kappa.view(-1, 1).double()
    theta = theta.view(-1, 1).double()
    sigma = sigma.view(-1, 1).double()
    v0 = v0.view(-1, 1).double()
    rho = rho.view(-1, 1).double()
    ...
    log_S = torch.full((num_samples, N_paths), math.log(S_val), dtype=torch.float64, device=device)
...
def Bates_MC_Batched(kappa, theta, sigma, v0, rho, lambdJ, muJ, sigmaJ,
    ...
    v0 = v0.view(-1, 1).double()
    rho = rho.view(-1, 1).double()
    lambdJ = lambdJ.view(-1, 1).double()
    muJ = muJ.view(-1, 1).double()
    sigmaJ = sigmaJ.view(-1, 1).double()
    ...
    for t in range(N_steps):
        ...
        jump_mask = (
            torch.rand(num_samples, N_paths, dtype=torch.float64, device=device) < jump_prob
        ).double()
        jump_sizes = (
            torch.randn(num_samples, N_paths, dtype=torch.float64, device=device) * sigmaJ + muJ
        ) * jump_mask
...
def Bergomi_1F_MC_Batched(xi, nu, rho, beta, T, K_strikes, S, r, q, N_paths=3000, N_steps=100):
    ...
    xi = xi.view(-1, 1).double()
    nu = nu.view(-1, 1).double()
    beta = beta.view(-1, 1).double()
    rho = rho.view(-1, 1, 1).double()
...
def rBergomi_MC_Batched(xi, nu, rho, H, T, K_strikes, S, r, q, N_paths=4000, N_steps=150):
    ...
    for t in range(N_steps):
        ...
        if t == 0:
            Z_t = torch.zeros(num_samples, N_paths, dtype=torch.float64, device=device)
        else:
...
def Bergomiprice_Batched(xi, nu, rho_SV, beta, T, K_strikes, S, r, q, N_paths=3000, N_steps=100):
    prices = Bergomi_1F_MC_Batched(
        xi=xi.double(), nu=nu.double(), rho=rho_SV.double(), beta=beta.double(),
...
def rBergomiprice_Batched(xi, nu, rho_SV, H, T, K_strikes, S, r, q, N_paths=4000, N_steps=150):
    prices = rBergomi_MC_Batched(
        xi=xi.double(), nu=nu.double(), rho=rho_SV.double(), H=H.double(),
        T=float(T), K_strikes=K_strikes, S=S, r=r, q=q,
        N_paths=N_paths, N_steps=N_steps
    )
    return prices.to(torch.float32)
